...
public final class ParserAndTranslator {
  ...
  private static void assignmentOrInvoc() throws SourceFileErrorException
  {
    ...
    if (getCurrentToken() != LPAREN) {
      ...
      if (t instanceof LocalVariableRec) {
         new PUSHLOCADDRinstr(t.offset); //added
      }
      else {
         new PUSHSTATADDRinstr(t.offset); //added
      }
  ...
  private static void whileStmt() throws SourceFileErrorException
  {
	...
	expr7();
	JUMPONFALSEinstr jumpInstr = new JUMPONFALSEinstr(Instruction.OPERAND_NOT_YET_KNOWN);
	accept(RPAREN);
	statement();
	
	new JUMPinstr(n); //jumps back to redo while loop
    ...
    switch (getCurrentToken()) {
		...
		case PRINT:
	       ...
	       case PRINTLN:
	      	 ...
	      	 {
	      		 printArgument();
	      	     new WRITELNOPinstr(); //this is the only change from assignment 1
	      	 }
	        accept(RPAREN);
	        accept(SEMICOLON);
	        break;
  ...
  private static void printArgument() throws SourceFileErrorException
  {
    ...
    /* added */
	if(getCurrentToken() == CHARSTRING_)
	{
		nextToken();
		int a = LexicalAnalyzer.getStartOfString();
		int b = LexicalAnalyzer.getEndOfString();
		WRITESTRINGinstr writeString = new WRITESTRINGinstr(a, b);
    }
	else {
		expr3();
		WRITEINTinstr writeInt = new WRITEINTinstr();
	}
  ...
  private static void expr5() throws SourceFileErrorException
  {
	...
	{
	    Symbols operator = getCurrentToken();
		nextToken();
	    expr4();
	    if(operator == EQ)
			new EQinstr();
